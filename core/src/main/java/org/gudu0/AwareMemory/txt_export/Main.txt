...
//        itemsSys = new ItemsSystem();
    ...
    private float getTileCost(int tile) {
        return switch (tile) {
            case WorldGrid.TILE_CONVEYOR -> COST_CONVEYOR;
            case WorldGrid.TILE_SMELTER  -> COST_SMELTER;
            case WorldGrid.TILE_SELLPAD  -> COST_SELLPAD;
            case WorldGrid.TILE_SPAWNER  -> COST_SPAWNER;
            case WorldGrid.TILE_CRUSHER -> COST_CRUSHER;
            case WorldGrid.TILE_SPLITTER_FL -> COST_SPLIT_FL;
            case WorldGrid.TILE_SPLITTER_FR -> COST_SPLIT_FR;
            case WorldGrid.TILE_SPLITTER_LR -> COST_SPLIT_LR;
            case WorldGrid.TILE_MERGER -> COST_MERGER;
    ...
    public void doPlacedTilesDraw() {
        ...
        for (int x = 0; x < worldCellsX(); x++) {
            for (int y = 0; y < worldCellsY(); y++) {
                ...
                switch (id) {
                    case WorldGrid.TILE_CONVEYOR -> {
                        var te = tileWorld.getEntity(x, y);
                        if (te instanceof ConveyorEntity c) {
                            ...
                            switch (c.getShape()) {
                                ...
                                case TURN_LEFT, TURN_RIGHT -> {
                                    ...
                                    Dir in = (c.getShape() == org.gudu0.AwareMemory.entities.ConveyorEntity.Shape.TURN_LEFT)
                                        ? out.left()
                                        : out.right();
    ...
    public void doDrawItems() {
        ...
        for (TileWorld.ItemRenderInfo info : tileWorld.renderInfos()) {
            Texture tex = switch (info.item().type) {
                case ORE -> oreTex;
                case DUST -> dustTex;
                case INGOT -> ingotTex;
            };
