...
public final class ConveyorEntity extends TileEntity {
    ...
    public int[] entryCellFrom(Dir fromEdge) {
        ...
        Dir in = switch (shape) {
            case STRAIGHT -> out.opposite(); // from behind
            case TURN_LEFT -> out.left();    // from left side
            case TURN_RIGHT -> out.right();  // from right side
        };
        ...
        int[] base = switch (shape) {
            case STRAIGHT -> new int[]{0, 2};
            case TURN_LEFT -> new int[]{2, 4};
            case TURN_RIGHT -> new int[]{2, 0};
        };
...
//    }
    ...
    private int[][] pathBase() {
        return switch (shape) {
            ...
            case TURN_LEFT -> PATH_LEFT_IN;
            case TURN_RIGHT -> PATH_RIGHT_IN;
        };
    }
